...
do	
	...
	python /share/lemaylab-backedup/Zeya/scripts/ARG_metagenome/non_pipeline/CAZy_db_analysis_counter.py --infile $output_dir_diamond/${STEM}.txt --output $output_dir_diamond/${STEM}_org_by_gene.csv
	...
	python /share/lemaylab-backedup/Zeya/scripts/ARG_metagenome/non_pipeline/make_CAZy_normtab.py --mc /share/lemaylab-backedup/Zeya/proceesed_data/NovaSeq043/step5_MicrobeCensus_merged/${STEM}_mc.txt --genelen /share/lemaylab-backedup/databases/CAZy/CAZyDB.07312019.uniq.len.txt --count $output_dir_diamond/${STEM}_org_by_gene.csv --out $output_dir_diamond/${STEM}_org_by_gene_norm.csv --mergein $file # This argument does not do anything here, but python2 requires *args to be not empty so I supply a dummy argument (if using python3, can omit this input)
done

python /share/lemaylab-backedup/Zeya/scripts/ARG_metagenome/non_pipeline/make_CAZy_normtab.py --mergeout $output_dir_diamond/merged_gene_norm_tab.csv --mergein $output_dir_diamond/*_org_by_gene_norm.csv 
